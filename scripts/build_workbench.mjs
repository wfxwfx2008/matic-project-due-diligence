...
applyBaseSheet(
  productMatrix,
  "具体产品与方案矩阵",
  "证据长表：每个“产品或方案×对比维度”单独一行并沿用稳定CMP-P编号；“具体判断”填写可进入报告横向矩阵的展示值，数值另注明单位、口径和条件，成文时再按维度转置。",
...
applyBaseSheet(
  routeMatrix,
  "技术路线矩阵",
  "证据长表：每个“技术路线×对比维度”单独一行并沿用稳定CMP-R编号；“具体判断”填写可进入报告横向矩阵的展示值，本项目路线必须登记，成文时再按维度转置。",
...
home.getRange("A20:A24").values = [
  ...
  ["3. 产品/方案与技术路线分别按证据长表维护；“具体判断”优先写成可直接进入报告单元格的展示值，并记录单位、口径及条件。"],
  ["4. 阶段性或最终报告只在明确指令后生成；成文时将有效长表转置为“维度为行、本项目为首列”的横向矩阵。"],
